...
while IFS=$'\t' read -r file_id name folder; do
    ...
    bcftools_cmd_ASSESS_1="tabix -p vcf ${name}; bcftools view -i 'ASSESS>=1 && FILTER=\"PASS\"' ${name} -o ${test_vcf_filtered_ASSESS_1}; bgzip ${test_vcf_filtered_ASSESS_1}"
    bcftools_cmd_ASSESS_2="tabix -p vcf ${name}; bcftools view -i 'ASSESS>=2 && FILTER=\"PASS\"' ${name} -o ${test_vcf_filtered_ASSESS_2}; bgzip ${test_vcf_filtered_ASSESS_2}"
    bcftools_cmd_ASSESS_3="tabix -p vcf ${name}; bcftools view -i 'ASSESS>=3 && FILTER=\"PASS\"' ${name} -o ${test_vcf_filtered_ASSESS_3}; bgzip ${test_vcf_filtered_ASSESS_3}"
    bcftools_cmd_ASSESS_4="tabix -p vcf ${name}; bcftools view -i 'ASSESS>=4 && FILTER=\"PASS\"' ${name} -o ${test_vcf_filtered_ASSESS_4}; bgzip ${test_vcf_filtered_ASSESS_4}"
    bcftools_cmd_ASSESS_5="tabix -p vcf ${name}; bcftools view -i 'ASSESS>=5 && FILTER=\"PASS\"' ${name} -o ${test_vcf_filtered_ASSESS_5}; bgzip ${test_vcf_filtered_ASSESS_5}"
