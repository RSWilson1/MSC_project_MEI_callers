...
def main():
    ...
    for hg_id in id_list:
        ...
        subprocess.run(['bgzip', f'{file_path}ALU.final_comp.vcf'])
        subprocess.run(['bgzip', f'{file_path}SVA.final_comp.vcf'])
        subprocess.run(['bgzip', f'{file_path}LINE1.final_comp.vcf'])
        subprocess.run(['tabix', '-p', 'vcf', f'{file_path}ALU.final_comp.vcf.gz'])
        subprocess.run(['tabix', '-p', 'vcf', f'{file_path}SVA.final_comp.vcf.gz'])
        subprocess.run(['tabix', '-p', 'vcf', f'{file_path}LINE1.final_comp.vcf.gz'])
