...
def main():
    ...
    2. Concatenates the ALU.final.vcf, SVA.final.vcf, and LINE1.final.vcf files into a single VCF file.
    ...
    for id in id_list:
        ...
        subprocess.run(['cd', f'/project/003_230901_MSc_MEI_detection/benchmarking_output/{id}/MELT/'])
        subprocess.run(['bcftools', 'concat', f'{id}_ALU.final.vcf', f'{id}_SVA.final.vcf', f'{id}_LINE1.final.vcf', '-o', f'{id}_MELT_concat.vcf'])
        subprocess.run(['bgzip', f'{id}_MELT_concat.vcf'])
        subprocess.run(['tabix', '-p', 'vcf', f'{id}_MELT_concat.vcf.gz'])
        print(f'{id} Finished')
