...
def process_url(ID, url):
    ...
    with open(log_filename, 'w') as log_file:
        ...
        subprocess.run(['bash', 'MELT_script_v0.2.1.sh', str(ID), str(url), str(filename)], stdout=log_file, stderr=subprocess.STDOUT)

if __name__ == '__main__':
    num_cores = os.cpu_count()
    with ThreadPoolExecutor(max_workers=num_cores) as executor:  # Adjust max_workers as needed
        # Submit tasks to the thread pool
        futures = {executor.submit(process_url, ID, url) for ID, url in links_dict.items()}
