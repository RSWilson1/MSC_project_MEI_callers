...
/usr/bin/time --verbose aria2c -x 16 -s 16 "${URL}" # -o "${ID}.bam"
/usr/bin/time --verbose aria2c -x 16 -s 16 "${URL}.bai" # -o "${ID}.bam.bai"
...
cluster_identifier \
    "scratch/${ID}/${filename}.bam" > "scratch/$ID/$ID.clusters.txt"
